...
const CREDENTIAL_CATEGORIES = [
  { id: 'military',       label: 'Military',           desc: 'Active duty, veterans, reservists, and military dependents',      docHint: 'Upload your military ID, DD-214, or VA card.' },
  { id: 'student',        label: 'Student',            desc: 'Currently enrolled college or university students',               docHint: 'Upload your student ID or current enrollment letter.' },
  { id: 'first_responder',label: 'First Responder',    desc: 'EMTs, firefighters, law enforcement, and 911 dispatchers',        docHint: 'Upload your official badge, ID, or employment letter.' },
  { id: 'teacher',        label: 'Teacher',            desc: 'K–12 educators and higher education faculty',                    docHint: 'Upload your school-issued ID or employment verification letter.' },
  { id: 'healthcare',     label: 'Healthcare Worker',  desc: 'Nurses, doctors, pharmacists, and medical staff',                docHint: 'Upload your medical license, hospital ID, or employment letter.' },
  { id: 'government',     label: 'Government Employee',desc: 'Federal, state, and local government employees',                 docHint: 'Upload your government-issued employee ID or pay stub.' },
  { id: 'senior',         label: 'Senior (55+)',        desc: 'Age 55 and older verification',                                  docHint: 'Upload a government-issued ID showing your date of birth.' },
  { id: 'nonprofit',      label: 'Nonprofit Worker',   desc: 'Employees of registered nonprofit organizations',                docHint: 'Upload your employee ID or a letter from your organization.' },
];
...
function CredentialWizard({ cred, onCancel, onSubmitted }) {
  ...
  async function handleSubmit() {
    ...
    setSubmitError(null);
    try {
      const result = await startCredentialVerification(cred.id, fieldValues, file);
      onSubmitted(result);
    } catch (err) {
    ...
    return (
      <div className="cp-wizard-body">
        ...
        <div className="cp-actions">
          ...
          <button type="button" className="cp-btn-primary" onClick={() => setStep(2)}>
      ...
      <div className="cp-wizard-body">
        ...
        <div className="cp-review-items">
          ...
          <div className="cp-review-item">
            ...
            <span className="cp-review-value">{file ? file.name : 'Not provided'}</span>
          </div>
        </div>
    ...
    <div className="cp-detail">
      ...
      <div className="cp-detail-header">
        ...
        {s === 'rejected' && userCred.reviewerNotes && (
          <div className="cp-rejected-banner">
            <strong>Reviewer note:</strong> {userCred.reviewerNotes}
          </div>
        )}
      ...
      <div className="cp-detail-sections">
        <div className="cp-detail-fields">
          ...
          {userCred.reviewerNotes && (
            <div className="cp-review-note-block">
              <div className="cp-detail-section-title" style={{ marginBottom: 8 }}>Reviewer Note</div>
              <div className="cp-review-note-text">{userCred.reviewerNotes}</div>
            </div>
          )}
        </div>
...
export default function Credentials() {
  ...
  function handleWizardSubmitted(result) {
    setCredentials((prev) => {
      const exists = prev.some((c) => c.credentialType === result.credentialType);
      return exists
        ? prev.map((c) => c.credentialType === result.credentialType ? result : c)
        : [...prev, result];
    });
    setActiveView(null);
  }
